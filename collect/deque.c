...
enum cc_stat deque_replace_at(Deque *deque, void *element, size_t index, void **out)
{
    ...
    i = (deque->first + index) & (deque->capacity - 1);
...
enum cc_stat deque_remove_first(Deque *deque, void **out)
{
    ...
    element = deque->buffer[deque->first];
    deque->first = (deque->first + 1) & (deque->capacity - 1);
    deque->size--;
...
enum cc_stat deque_remove_last(Deque *deque, void **out)
{
    ...
    last    = (deque->last - 1) & (deque->capacity - 1);
    element = deque->buffer[last];
    deque->last = last;
    deque->size--;
...
enum cc_stat deque_copy_shallow(Deque const * const deque, Deque **out)
{
    ...
    if (!copy)
        return CC_ERR_ALLOC;
    copy->buffer = deque->mem_alloc(deque->capacity * sizeof(void*));
    if (!(copy->buffer))
    {
        deque->mem_free(copy);
        return CC_ERR_ALLOC;
...
enum cc_stat deque_trim_capacity(Deque *deque)
{
    ...
    new_size = upper_pow_two(deque->size);

    if (new_size == deque->capacity)
        return CC_OK;

    new_buff = deque->mem_alloc(sizeof(void*) * new_size);
...
static enum cc_stat expand_capacity(Deque *deque)
{
    ...
    new_capacity = deque->capacity << 1;
    new_buffer = deque->mem_calloc(new_capacity, sizeof(void*));
 ...
 */
static size_t upper_pow_two(size_t n)
{
    if (n >= MAX_POW_TWO)
        return MAX_POW_TWO;
...
enum cc_stat deque_iter_next(DequeIter *iter, void **out)
{
    ...
    i = (iter->deque->first + iter->index) & c;
...
enum cc_stat deque_iter_remove(DequeIter *iter, void **out)
{
    ...
    status = deque_remove_at(iter->deque, iter->index, &rm);
    if (status == CC_OK)
    {
        iter->index--;
...
enum cc_stat deque_zip_iter_next(DequeZipIter *iter, void **out1, void **out2)
{
    ...
    d2_capacity = (iter->d2->capacity - 1);
    d2_last     = (iter->d2->last) & d2_capacity;
    d2_first    = (iter->d2->first) & d2_capacity;

    if (d2_last == d2_first || iter->index >= iter->d2->size)
        return CC_ITER_END;

    d1_index = (iter->d1->first + iter->index) & d1_capacity;
    d2_index = (iter->d2->first + iter->index) & d2_capacity;
