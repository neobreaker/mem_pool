...
static void mem_reset_tbl_8(mem_dev_t *mem_dev, char *p)
{
    ...
    if(offset < mem_dev->mem_valid_size)
    {
        ...
        for(i =0; i < num_block; ++i)
        {
			CC_ASSERT("[MEM_POOL]:  mem leak == 0 ", num_block ==  mem_dev->mem_tbl.tbl_w8[tbl_offset+i]);
...
static void mem_reset_tbl_16(mem_dev_t *mem_dev, char *p)
{
    ...
    if(offset < mem_dev->mem_valid_size)
    {
        ...
        for(i =0; i < num_block; ++i)
        {
			CC_ASSERT("[MEM_POOL]:  mem leak == 0 ", num_block ==  mem_dev->mem_tbl.tbl_w16[tbl_offset+i]);
...
static void mem_reset_tbl_32(mem_dev_t *mem_dev, char *p)
{
    ...
    {
        tbl_offset = offset/mem_dev->block_bytes;
        num_block = mem_dev->mem_tbl.tbl_w32[tbl_offset];
        for(i =0; i < num_block; ++i)
        {
			CC_ASSERT("[MEM_POOL]:  mem leak == 0 ", num_block ==  mem_dev->mem_tbl.tbl_w32[tbl_offset+i]);
            mem_dev->mem_tbl.tbl_w32[tbl_offset+i] = 0;
        }
        mem_dev->mem_free_size += num_block*mem_dev->block_bytes;
    }
}
